...
Authorization must be obtained from the user, and maintained over time. This is done using an
OAuth 2 flow.

1. The user starts at the website, accessing an auth page.
1. The HTTP server generates a random UUID and stores it in PostgreSQL.
   This UUID is used to generate an auth link on the server (as its state),
   and the user is redirected to it. The row stores extra metadata, including
...
1. The server verifies the state in PostgreSQL, removing it atomically.
    1. If the host of the request does not match the host of the state, then the state is
       stored back into PostgreSQL, and the request is redirected to the same URL at the correct
       host.
