...
func TestHelixFollowChannel(t *testing.T) {
	...
	newToken, err := tw.HelixFollowChannel(ctx, c.ID.AsInt64(), tok, 200)
	assert.NilError(t, err)
	assert.Assert(t, newToken == nil)

	newToken, err = tw.HelixFollowChannel(ctx, c.ID.AsInt64(), tok, 401)
	assert.Equal(t, err, twitch.ErrNotAuthorized)
	assert.Assert(t, newToken == nil)

	newToken, err = tw.HelixFollowChannel(ctx, c.ID.AsInt64(), tok, 404)
	assert.Equal(t, err, twitch.ErrNotFound)
	assert.Assert(t, newToken == nil)

	newToken, err = tw.HelixFollowChannel(ctx, c.ID.AsInt64(), tok, 422)
	assert.Equal(t, err, twitch.ErrUnknown)
	assert.Assert(t, newToken == nil)

	newToken, err = tw.HelixFollowChannel(ctx, c.ID.AsInt64(), tok, 500)
	assert.Equal(t, err, twitch.ErrServerError)
	assert.Assert(t, newToken == nil)

	newToken, err = tw.HelixFollowChannel(ctx, c.ID.AsInt64(), nil, 500)
	assert.Equal(t, err, twitch.ErrNotAuthorized)
	assert.Assert(t, newToken == nil)

	newToken, err = tw.HelixFollowChannel(ctx, c.ID.AsInt64(), tok, 901)
	assert.ErrorContains(t, err, errTestBadRequest.Error())
	assert.Assert(t, newToken == nil)
}
