...
func (r *variableR) GetChannel() *Channel {
	if r == nil {
		return nil
	}
	return r.Channel
...
var (
	...
	variableGeneratedColumns      = []string{}
)
...
func (variableL) LoadChannel(ctx context.Context, e boil.ContextExecutor, singular bool, maybeVariable interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeVariable.(*Variable)
		if !ok {
			object = new(Variable)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeVariable)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeVariable))
			}
		}
	} else {
		s, ok := maybeVariable.(*[]*Variable)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeVariable)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeVariable))
			}
		}
	}
...
func Variables(mods ...qm.QueryMod) variableQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"variables\".*"})
	}

	return variableQuery{q}
}
...
func (o *Variable) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(variableAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(variablePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert variables, could not build conflict column list")
			}

			conflict = make([]string, len(variablePrimaryKeyColumns))
			copy(conflict, variablePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"variables\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o VariableSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "DELETE FROM \"variables\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, variablePrimaryKeyColumns, len(o))
...
func (o *Variable) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return VariableExists(ctx, exec, o.ID)
}
