...
func (o *Highlight) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(highlightAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(highlightPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert highlights, could not build conflict column list")
			}

			conflict = make([]string, len(highlightPrimaryKeyColumns))
			copy(conflict, highlightPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"highlights\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o HighlightSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "DELETE FROM \"highlights\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, highlightPrimaryKeyColumns, len(o))
