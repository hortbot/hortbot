	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(quotePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert quotes, could not build conflict column list")
			}

			conflict = make([]string, len(quotePrimaryKeyColumns))
			copy(conflict, quotePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"quotes\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o QuoteSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "DELETE FROM \"quotes\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, quotePrimaryKeyColumns, len(o))
