...
func (o *CommandList) GetChannel() *Channel {
	if o == nil {
		return nil
	}
...
func (o *CommandList) GetCommandInfo() *CommandInfo {
	if o == nil {
		return nil
	}
...
func (commandListL) LoadChannel(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCommandList any, mods queries.Applicator) error {
...
func (commandListL) LoadCommandInfo(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCommandList any, mods queries.Applicator) error {
	var slice []*CommandList
	var object *CommandList
	...
	query := NewQuery(
		...
		qm.WhereIn(`command_infos.command_list_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *CommandList) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(commandListAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(commandListPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert command_lists, could not build conflict column list")
			}

			conflict = make([]string, len(commandListPrimaryKeyColumns))
			copy(conflict, commandListPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"command_lists\"", updateOnConflict, ret, update, conflict, insert, opts...)
