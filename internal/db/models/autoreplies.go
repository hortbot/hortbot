...
func (o *Autoreply) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(autoreplyAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(autoreplyPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert autoreplies, could not build conflict column list")
			}

			conflict = make([]string, len(autoreplyPrimaryKeyColumns))
			copy(conflict, autoreplyPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"autoreplies\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o AutoreplySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "DELETE FROM \"autoreplies\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, autoreplyPrimaryKeyColumns, len(o))
...
func (o *Autoreply) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AutoreplyExists(ctx, exec, o.ID)
}
