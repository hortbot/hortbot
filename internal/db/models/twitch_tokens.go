...
func (o *TwitchToken) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(twitchTokenAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(twitchTokenPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert twitch_tokens, could not build conflict column list")
			}

			conflict = make([]string, len(twitchTokenPrimaryKeyColumns))
			copy(conflict, twitchTokenPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"twitch_tokens\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o TwitchTokenSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "DELETE FROM \"twitch_tokens\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, twitchTokenPrimaryKeyColumns, len(o))
