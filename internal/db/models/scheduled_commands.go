...
func (o *ScheduledCommand) GetChannel() *Channel {
	if o == nil {
		return nil
	}
...
func (r *scheduledCommandR) GetChannel() *Channel {
	if r == nil {
		return nil
	}
...
func (o *ScheduledCommand) GetCommandInfo() *CommandInfo {
	if o == nil {
		return nil
	}
...
func (r *scheduledCommandR) GetCommandInfo() *CommandInfo {
	if r == nil {
		return nil
	}
...
var (
	...
	scheduledCommandGeneratedColumns      = []string{}
)

type (
	// ScheduledCommandSlice is an alias for a slice of pointers to ScheduledCommand.
	// This should almost always be used instead of []ScheduledCommand.
...
func (scheduledCommandL) LoadChannel(ctx context.Context, e boil.ContextExecutor, singular bool, maybeScheduledCommand any, mods queries.Applicator) error {
	var slice []*ScheduledCommand
	var object *ScheduledCommand

	if singular {
		var ok bool
		object, ok = maybeScheduledCommand.(*ScheduledCommand)
		if !ok {
			object = new(ScheduledCommand)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeScheduledCommand)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeScheduledCommand))
			}
		}
	} else {
		s, ok := maybeScheduledCommand.(*[]*ScheduledCommand)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeScheduledCommand)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeScheduledCommand))
			}
		}
	}
...
// loaded structs of the objects. This is for an N-1 relationship.
func (scheduledCommandL) LoadCommandInfo(ctx context.Context, e boil.ContextExecutor, singular bool, maybeScheduledCommand any, mods queries.Applicator) error {
	var slice []*ScheduledCommand
	var object *ScheduledCommand
...
func ScheduledCommands(mods ...qm.QueryMod) scheduledCommandQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"scheduled_commands\".*"})
	}

	return scheduledCommandQuery{q}
}
...
func (o *ScheduledCommand) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(scheduledCommandAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(scheduledCommandPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert scheduled_commands, could not build conflict column list")
			}

			conflict = make([]string, len(scheduledCommandPrimaryKeyColumns))
			copy(conflict, scheduledCommandPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"scheduled_commands\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *ScheduledCommand) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return ScheduledCommandExists(ctx, exec, o.ID)
}
