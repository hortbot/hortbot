...
func (o *BlockedUser) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(blockedUserAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(blockedUserPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert blocked_users, could not build conflict column list")
			}

			conflict = make([]string, len(blockedUserPrimaryKeyColumns))
			copy(conflict, blockedUserPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"blocked_users\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o BlockedUserSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "DELETE FROM \"blocked_users\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, blockedUserPrimaryKeyColumns, len(o))
