...
func (o *RepeatedCommand) GetChannel() *Channel {
	if o == nil {
		return nil
	}
...
func (o *RepeatedCommand) GetCommandInfo() *CommandInfo {
	if o == nil {
		return nil
	}

	return o.R.GetCommandInfo()
}
...
func (repeatedCommandL) LoadChannel(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRepeatedCommand any, mods queries.Applicator) error {
	var slice []*RepeatedCommand
	var object *RepeatedCommand

	if singular {
		var ok bool
		object, ok = maybeRepeatedCommand.(*RepeatedCommand)
		if !ok {
			object = new(RepeatedCommand)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeRepeatedCommand)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeRepeatedCommand))
			}
		}
	} else {
		s, ok := maybeRepeatedCommand.(*[]*RepeatedCommand)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeRepeatedCommand)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeRepeatedCommand))
			}
		}
	}
...
// loaded structs of the objects. This is for an N-1 relationship.
func (repeatedCommandL) LoadCommandInfo(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRepeatedCommand any, mods queries.Applicator) error {
	var slice []*RepeatedCommand
	var object *RepeatedCommand
	...
	if singular {
		...
		args[object.CommandInfoID] = struct{}{}
	...
	query := NewQuery(
		...
		qm.WhereIn(`command_infos.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *RepeatedCommand) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(repeatedCommandAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(repeatedCommandPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert repeated_commands, could not build conflict column list")
			}

			conflict = make([]string, len(repeatedCommandPrimaryKeyColumns))
			copy(conflict, repeatedCommandPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"repeated_commands\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *RepeatedCommand) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return RepeatedCommandExists(ctx, exec, o.ID)
}
